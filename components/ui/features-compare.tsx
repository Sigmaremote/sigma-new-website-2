...
export function FeaturesCompare({
  ...
  // --- measure the actual position of the emphasized column ---
  ...
  React.useLayoutEffect(() => {
    ...
    // find the first row's cell for the emphasized column
    const selector = `[data-row="0"][data-col="${emphasizeIndex}"]`;
      ...
      <div className="mx-auto max-w-6xl rounded-2xl px-5 pb-8 pt-6" style={{ background: BG }}>
        ...
        <div className="mx-auto mt-4 max-w-5xl hidden sm:block">
          <div className="relative" ref={tableRef}>
            {/* responsive highlight that matches the real column */}
            {hilite && (
              <motion.div
                aria-hidden="true"
                ...
                >
                  ...
                  {r.values.map((v, vi) => (
                    ...
                    >
                      {v ? (
                        <motion.span
                          className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[rgba(207,248,106,0.18)]"
                          title="Included"
                          aria-label={`${columns[vi]}: included`}
                        ...
                        <motion.span
                          className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[rgba(255,92,92,0.15)] border border-white/10"
                          title="Not included"
                          aria-label={`${columns[vi]}: not included`}
                          whileHover={{ scale: 1.1 }}
                          transition={{ type: "spring", stiffness: 400, damping: 10 }}
                        >
                          <motion.div
